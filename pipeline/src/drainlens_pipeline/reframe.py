...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Re-express an artefact's coordinates in a larger extent's frame.",
    )
    parser.add_argument("--in", dest="source_file", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--from", dest="source", choices=sorted(EXTENTS), required=True)
    parser.add_argument("--to", dest="target", choices=sorted(EXTENTS), required=True)
    args = parser.parse_args(argv)

    artefact = json.loads(args.source_file.read_text(encoding="utf-8"))
    moved = reframe(artefact, EXTENTS[args.source], EXTENTS[args.target])
    ...
    print(
        f"wrote {args.out}: {shapes} shapes moved from {args.source} into {args.target}",
        file=sys.stderr,
    )
    return 0
