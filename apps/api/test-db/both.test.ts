...
describe('the council extent in the database', () => {
  ...
  it('holds derived layers built over the whole council, in its frame', async () => {
    /*
     ...
     * for. Kensington's square is the council's 1500-2500 E, 6000-7000 N.
     *
     * This asserted the first channel lay past (1500, 6000), which held while
     * the layers were Kensington's moved into this frame. Since 13 September
     * they are built council-wide from one terrain run, so what has to be true
     * is that they reach well beyond that square -- a Kensington-frame
     * artefact would stop at 1000 m -- and that the list of tiles the archive
     * lacks survives the database, because it is the only record of where
     * nothing is claimed.
     */
    const derived = await derivedArtefact(client, 'city-of-melbourne');
    expect(derived.extent.width_m).toBe(8500);

    // Reduced rather than spread: the council's channels are tens of
    // thousands of vertices, past what an argument list should carry.
    const eastings = (derived.layers.channel ?? []).flatMap((line) => line.c.map((point) => point[0]));
    expect(eastings.reduce((low, e) => Math.min(low, e), Infinity)).toBeLessThan(1500);
    expect(eastings.reduce((high, e) => Math.max(high, e), -Infinity)).toBeGreaterThan(7000);

    const envelope = derived as typeof derived & { missing_tiles?: string[]; covers?: string };
    expect(envelope.missing_tiles).toHaveLength(95);
    expect(envelope.covers).toContain('nothing is claimed');
  });
