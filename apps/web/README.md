...
**The map** (`src/map/`) — the viewport transform, hit testing, drawing, and the React component that binds them to a canvas. Roads, pipes, pits and street labels; Ground height beneath them; and the three derived layers over the top. Ground height is `terrain.ts`'s fixed AHD ramp, drawn from pre-coloured 500 m tiles for the whole council (`terrainTiles.ts`), with contours and spot heights from `terrainMarks.ts`; it replaced a layer painted from whole elevation arrays on 14 September. `places.ts` writes fourteen suburb names on the zoomed-out map, anchored in MGA55 so they land right on either extent, and fades them out as the street names arrive. `nearby.ts` measures to the true nearest point of the closest derived path and low area — *at or very near* within 5 m, or *within* a low area the address sits in — and `addressGround.ts` reads which way the ground falls around the address, so `AddressInsight.tsx` can say both in words and in one small figure without implying the one leads to the other. `difference.ts` paints where a finished comparison puts more water than its baseline — flat violet, because AD7 allows one output and it is *where*, not how much, so a ramp would invite reading a quantity off a legend this product does not publish.
...
**The flood history** (`src/history/`) — the artefact behind the board, and the checks that refuse it. Those checks are strict about *sentences* as well as numbers: no reporting period, no geographic unit, no source, or no note saying what a count is, and the page does not render. A ranked list of suburbs with nothing qualifying it is the one shape that page must never take. `board.ts` writes the units beside every number and orders the second ranking, by call-outs per 1,000 residents, without recomputing the rate: `severity.ts` is the one place that is calculated. `drawAreas.ts` fills the 281 statistical areas as their own boundaries, and `events.ts` lets a verified flood event reach the screen only once a team member's name and date are on it — the filter is the process, which is why it sits in the loader rather than the component. `useAreas.ts` loads the areas, their boundaries, the population and the events once, lazily, for the board and the flood map together.

**The data credit** (`src/ui/attribution.ts`) — read from the artefacts. It was folded in the footer from 14 September under *Data sources · Licensing · Not a flood warning*; since 16 September it is on the **About the data** page (`src/ui/sources.ts`, `src/ui/SourcesPanel.tsx`), opened by *Data sources and limits ›* in the footer of every screen and by grey source links in context (`SOURCE_LINKS`). CC BY 4.0 requires the attribution to be visible to the person using the work, which one press still makes it, and it includes the clause people skip: an indication that changes were made.

**One name for each thing** (`src/ui/terms.ts`) — the layer names, the three sources a value can come from, and the flood map's two measures, read by badges, legends, buttons and body text alike. `terms.test.ts` keeps the retired names out of the source, which is why *Severity Score* and *Official recorded data* appear nowhere a resident reads them. Model identifiers such as the `'severity'` mode and `rate` keep their old names.
...
**Iteration 2 is Epic 3 and Epic 4 — 134 sub-criteria against 96** — and its criteria and work breakdown are in [ITERATION-2-ACCEPTANCE.md](../../docs/ITERATION-2-ACCEPTANCE.md) and [ITERATION-2-TASKS.md](../../docs/ITERATION-2-TASKS.md). Counting what is met here would be a second copy of a checklist that already exists; the honest summary is that Epic 3 is a built feature being reconnected and extended, and Epic 4 is a new one, now on screen as the flood map, the ranking by rate and the verified events.
...
| `derived.json` | 170 KB | `drainlens_pipeline.derived` |
...
The three council artefacts are not here. They are in `apps/api/data/city-of-melbourne/`, where the API's Dockerfile copies them from, because `/data` is ignored by both git and Docker: `map.json` 6.67 MB, `trace.json` 693 KB, `derived.json` 7.38 MB since it was rebuilt from the council-wide ground on 13 September.

`sa2-areas.json`, `population.json`, `sa2-points.json` and `flood-events.json` are Epic 4’s, and the flood map and the board's second ranking read them. `sa2-areas.json` is every statistical area in Greater Melbourne rather than the board’s top thirty, because a map of thirty implies the other 251 are empty and 245 of them are not; `population.json` is the denominator of *SES flood call-outs per 1,000 residents*, the measure the criteria call the Severity Score; and `sa2-points.json` carries each area's simplified boundary as well as the point its name is written at. The board keeps its own file and its own cap: AC 2.2.1.b limits it to thirty locations, and that limit is held in the data rather than in a screen. `tools/data/check-areas.mjs` asserts the thirty are the top of the 281 and agree with them field for field, and that every area has a boundary with its name point inside it.
