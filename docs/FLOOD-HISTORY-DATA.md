...
> **14 September: the screens say *call-out* where this file says *dispatch*.** The copy review (#144) chose the plainer word, and the board and the flood map now write *SES flood call-outs*, and both say *Each count is one SES crew response, not one flood event*. The meaning is the one measured above — one call-out is one crew dispatch — and nothing about the counting changed. `flood-history.json` still says *crew dispatches* in its `note`, which is a pipeline change not yet made.
>
> **15 September: *emergency responses*, and the crew sentence corrected.** The second copy audit replaced *call-outs* with *emergency responses* and spelled out *Victoria State Emergency Service (SES)* on each page. It suggested explaining a count as *each crew sent counts as one emergency response*; the data quality statement quoted above says multiple crew attendances at one incident are precluded, so the screens say *One emergency response is one flood job the SES sent crews to, however many crews went* (`FLOOD.explain`). The area record's old *Several crews at one incident count once each* was wrong in the same direction and is gone. The board has also gained a second ranking, per 1,000 residents over the 274 areas with a population to divide by (#145); that rate is defined in [SEVERITY-SCORE.md](./SEVERITY-SCORE.md), not here.
>
> **16 September: a shorter sentence, still accurate.** Copy audit v4 changed `FLOOD.explain` to *Each one is a time the SES was sent to help.* That is one dispatch to one job, which is what one count is, and it makes no claim about how many crews went, so it cannot be read as one count per crew. The flood map's ⓘ beside an area's total adds *not one flood*.
