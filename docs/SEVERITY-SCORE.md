...
> **14 September: the mode no longer keeps the criteria's name.** The copy review (#144) renamed it *SES flood call-outs per 1,000 residents*, and a dispatch is written as a *call-out*, so the value reads, for example, `4.71+ call-outs per 1,000 residents`. The argument above is why that was an easy change to accept: the unit was already the name's better half. *Severity Score* is now the criteria's term and nothing on screen, and `ui/terms.test.ts` fails if it returns; the mapping is in [ITERATION-2-ACCEPTANCE.md](./ITERATION-2-ACCEPTANCE.md) under *Names on screen*. This file keeps its title because the criteria it answers keep theirs.
...
- **Rounding.** Two decimal places (`scoreLabel`), with a `+` where a count inside the area was withheld. An area with no score says *No rate* — *No score* until 14 September — and shows its residents, so it cannot be read as a low score.
...
- **The board ranks by it** (#145). The flood history board can be *Ranked by* either *Call-outs* or *Call-outs per 1,000 residents* — the two questions in *What normalising actually changes* as a switch — over the 274 areas with a rate, and *How the rate is calculated* sets out the three steps above with a worked example divided out from a real area, the bands from `SEVERITY_BREAKS` and the 1,000-resident minimum from `population.json`, so none of it is typed twice.
