...
- [x] **Loaded** → *4.1.3.a*. 1,967 rows, 281 areas across seven years, at SA2 as the comment asked. **But the database cannot join it to the counts yet**: `flood_area` holds the published thirty by name and `population` holds 281 by code. Publishing every in-scope area with its code is the next move and is a change to an artefact the board already reads
