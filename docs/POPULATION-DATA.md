...
- **The artefacts now join; the database still does not.** `sa2-areas.json` publishes all 281 areas with their ASGS code, so the counts and the denominator meet on a key, and `tools/data/check-areas.mjs` fails if they stop agreeing. Inside Postgres, `flood_area` is still the published thirty keyed by name — loading the scope list, and deciding whether the board’s cap becomes a `rank` column or stays a separate table, is the next piece.
