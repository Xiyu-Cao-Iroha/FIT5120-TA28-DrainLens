...
- **It is loaded.** `drainlens_pipeline.population` writes the artefact and `apps/api/src/load.ts` puts 1,967 rows in — 281 areas across seven 30 Junes, keyed by the nine-digit code at `'SA2'`. The seven areas under the threshold are loaded too: a population is a fact, and the rule about what may be divided belongs to whoever divides.
- **The counts and the denominator cannot yet be joined in the database.** `population` holds all 281 areas by code; `flood_area` holds the published **thirty** by name. There is no column between them and not the same number of rows, so the score cannot be computed from these tables as they stand. The flood artefact would have to publish every in-scope area and carry the code beside the name — a change to something the board already reads, and its own piece of work.
