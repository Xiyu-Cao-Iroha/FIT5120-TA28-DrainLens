...
- **They join in the database too.** `flood_area_coverage.sa2_code` carries the ASGS code beside the name, and the flood tables hold all 281 areas rather than the board's thirty, with `board_rank` marking the thirty — so AC 2.2.1.b stays a property of the rows rather than becoming a `LIMIT` in a query. The Severity Score's own query runs in `apps/api/test-db/load.test.ts` and returns the three areas [SEVERITY-SCORE.md](./SEVERITY-SCORE.md) names.
